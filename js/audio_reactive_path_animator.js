/**
 * File: audio_reactive_path_animator.js
 * Adapted from FL_PathAnimator.js for AudioReactivePathAnimator
 */
...
app.registerExtension({
    name: "FillNodes.AudioReactivePathAnimator",
    async nodeCreated(node) {
        if (node.comfyClass === "AudioReactivePathAnimator") {
            // Find the paths_data widget immediately
            const pathsDataWidget = node.widgets.find(w => w.name === "paths_data");
            if (!pathsDataWidget) {
                console.error("AudioReactivePathAnimator: 'paths_data' widget not found!");
                return;
            }
...
function openPathEditor(node, pathsDataWidget) {
    ...
    // CHANGED: Use screen_width/height instead of frame_width/height
    const frameWidthWidget = node.widgets.find(w => w.name === "screen_width");
    const frameHeightWidget = node.widgets.find(w => w.name === "screen_height");
...
class PathEditorModal {
    ...
    setupKeyboardHandlers() {
        this.keydownHandler = (e) => {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key === 'v') {
                console.log('AudioReactivePathAnimator: Ctrl+V detected');
                e.preventDefault();
                e.stopPropagation();
        ...
        this.pasteHandler = (e) => {
            console.log('AudioReactivePathAnimator: Paste handler called');
            e.preventDefault();
            e.stopPropagation();
            this.handlePaste(e);
        };
        ...
        console.log('AudioReactivePathAnimator: Keyboard handlers registered');
    }
    ...
    attachPasteListener() {
        ...
        if (this.container) {
            ...
            console.log('AudioReactivePathAnimator: Paste handlers attached to container and document');
        }
    }
    ...
    async pasteFromClipboard() {
        console.log('AudioReactivePathAnimator: Attempting to read from clipboard using Clipboard API');

        try {
            // Check if Clipboard API is available
            if (!navigator.clipboard || !navigator.clipboard.read) {
                console.log('AudioReactivePathAnimator: Clipboard API not available, falling back to paste event');
                return;
            }

            const clipboardItems = await navigator.clipboard.read();
            console.log('AudioReactivePathAnimator: Read', clipboardItems.length, 'items from clipboard');

            for (const clipboardItem of clipboardItems) {
                console.log('AudioReactivePathAnimator: Clipboard item types:', clipboardItem.types);

                for (const type of clipboardItem.types) {
                    if (type.startsWith('image/')) {
                        console.log('AudioReactivePathAnimator: Found image type:', type);
            ...
            console.log('AudioReactivePathAnimator: No image found in clipboard');
        } catch (err) {
            console.error('AudioReactivePathAnimator: Error reading from clipboard:', err);
            console.log('AudioReactivePathAnimator: You may need to grant clipboard permission');
        }
    }

    loadImageFromBlob(blob) {
        console.log('AudioReactivePathAnimator: Loading image from blob, size:', blob.size);
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                this.render();
                console.log('AudioReactivePathAnimator: Image pasted successfully -', img.width, 'x', img.height);
            };
            img.onerror = () => {
                console.error('AudioReactivePathAnimator: Failed to load pasted image');
            };
            img.src = event.target.result;
        };
        reader.onerror = () => {
            console.error('AudioReactivePathAnimator: Failed to read image blob');
        };
        reader.readAsDataURL(blob);
    }

    handlePaste(e) {
        console.log('AudioReactivePathAnimator: Paste event triggered');

        // Get clipboard items
        const items = e.clipboardData?.items;
        if (!items) {
            console.log('AudioReactivePathAnimator: No clipboard items found');
            return;
        }

        console.log('AudioReactivePathAnimator: Clipboard has', items.length, 'items');

        // Look for image items
        for (let i = 0; i < items.length; i++) {
            const item = items[i];
            console.log('AudioReactivePathAnimator: Item', i, 'type:', item.type);

            // Check if item is an image
            if (item.type.indexOf('image') !== -1) {
                console.log('AudioReactivePathAnimator: Found image in clipboard via paste event');
                const blob = item.getAsFile();
                if (!blob) {
                    console.log('AudioReactivePathAnimator: Failed to get blob from clipboard item');
                    continue;
                }
    ...
    show() {
        ...
        setTimeout(() => {
            ...
            console.log('AudioReactivePathAnimator: Container focused');
        }, 100);
    }
    ...
    close() {
        ...
        console.log('AudioReactivePathAnimator: All event listeners removed');
